...
auto create_json(const std::string& method, std::vector<rapidjson::Value>& params) -> std::string {
    rapidjson::Document input {};
    auto& allocator = input.GetAllocator();
    // create JSON RPC call for Bitcoin Core RPC
    input.SetObject();
    ...
    input.AddMember("id",
		    ...
		    .SetString(id.c_str(), id.size(), allocator).Move(),
		     allocator);
    input.AddMember("method",
		    rapidjson::Value{rapidjson::kStringType}
		    .SetString(method.c_str(), method.size(), allocator).Move(),
		     allocator);
	...
	-> std::unique_ptr<rapidjson::Document>
{
    ...
    curl_write_callback cb = [](char* response, size_t size, size_t nmemb, void* userp) -> size_t {
...
auto estimate_fee(const fullnode_params_t& fullnode_params, int conf_target) -> uint64_t {
    ...
    std::vector<rapidjson::Value> params {};
    rapidjson::Value conf_target_node {rapidjson::kNumberType}; conf_target_node.SetInt(conf_target);
    ...
    double sats_per_byte = std::ceil(SATOSHIS_PER_BTC * feerate_as_double / 1024l);

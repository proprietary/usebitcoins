...
void test_bip32() noexcept {
    ...
    memcpy(chain_code_master,
           ...
           32);
    BOOST_CHECK_EQUAL(node.fingerprint, 0x00000000);
    BOOST_CHECK_EQUAL(node.chain_code, chain_code_master, 32);
    BOOST_CHECK_EQUAL(node.private_key, private_key_master, 32);
...
static void RunTest() {
    ...
    unsigned int n_child = 0x80000000;
    ...
    key.Encode(data);
    pubkey.Encode(data);
    BOOST_CHECK(EncodeExtKey(key) == priv);
}

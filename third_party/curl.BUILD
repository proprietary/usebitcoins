...
genrule(
	...
	"#  define HAVE_BORINGSSL 1",
        ...
        "#else",
	# TODO: CA bundle path needs to be cross-distro
	"#  define CURL_CA_BUNDLE \"/etc/ssl/certs/ca-bundle.crt\"",
...
cc_library(
    ...
    srcs = [
	":configure",
	] + glob(["lib/**/*.h", "lib/**/*.c"], exclude = ["lib/config-*.h"]),
    # + select({
    ...
    copts = select({
        ...
        "//conditions:default": [
            "-Iexternal/se_haxx_curl_curl/lib",
            "-D_GNU_SOURCE",
            "-DBUILDING_LIBCURL",
            "-DHAVE_CONFIG_H",
            "-DCURL_DISABLE_FTP",
    ...
    defines = ["CURL_STATICLIB", "CURL_NO_OLDIES"],
    includes = ["include"],
    linkopts = select({
